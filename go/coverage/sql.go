...
import (
	...
	"fmt"
	"os"
	shSql "sparrowhawktech/toolkit/sql"
...
type Config struct {
	DatasourceConfig                 *shSql.DatasourceConfig `json:"datasourceConfig"`
...
func ExecuteDB(config shSql.DatasourceConfig, delegate func(db *sql.DB)) {
	db := shSql.OpenDB(config)
	defer shSql.CloseDB(db)
	delegate(db)
}
