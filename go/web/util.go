...
func ResolveErrorMessage(r interface{}, defaultMessage string) string {
	if err, ok := r.(*url.Error); ok {
		...
		return err.ErrorMessage
	} else if err, ok := r.(errors.Error); ok {
		message := errors.GlobalDictionary.FindErrorDefinition(err.ErrorCode).Message
		return *message
	} else {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		if fd.IsExported() {
			...
			if ok && tv == "true" && f.IsNil() {
				errors.ThrowError(requiredFieldMissingErrorCode, fmt.Sprintf("%s.%s", path, fd.Name))
			}
