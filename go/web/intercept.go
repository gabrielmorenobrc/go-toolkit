...
func catchFatal(writer http.ResponseWriter, r *http.Request) {
	if e := recover(); e != nil {
		...
		if err, ok := e.(errors.Error); ok {
			writer.Header().Set("Content-Type", ContentTypeApplicationJson)
			errorDefinition := errors.GlobalDictionary.FindErrorDefinition(err.ErrorCode)
			responsePayload := DictionaryErrorResponse{
				ErrorCode:    err.ErrorCode,
				ErrorMessage: *errorDefinition.Message,
				Data:         err.Data,
			}
			writer.Header().Set(HeaderContentType, ContentTypeApplicationJson)
			util.JsonEncode(responsePayload, writer)
		} else if err, ok := e.(util.FriendlyErrorMessage); ok {
			writer.Header().Set(HeaderContentType, ContentTypeApplicationJson)
